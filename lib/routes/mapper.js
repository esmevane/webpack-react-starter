...
class AsyncRouteNode {
  constructor({ context, component, index, path, routes }) {
    const toNode = item =>
      item instanceof Route ?
        item.toAsyncRouteNode(context) :
        item
  ...
  toObject() {
    ...
    return {
      path,
      [ this._getComponentKey() ]: this._getAsyncFetch(),
  ...
  _getAsyncFetch() {
    ...
    const fetchComponent = (location, callback) =>
      require.ensure([], () => callback(null, context(component).default))
...
class Route {
  ...
  toAsyncRouteNode(context) {
    return new AsyncRouteNode({ ...this, context }).toObject()
  }
